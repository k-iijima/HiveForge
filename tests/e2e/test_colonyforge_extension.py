#!/usr/bin/env python3
"""ColonyForge VS Code拡張のビジュアルテスト

Agent UIを使用してVS Code内のColonyForge拡張機能をテストします。
...
class ColonyForgeExtensionTest:
    """ColonyForge拡張機能のテストクラス"""
    ...
    async def click_activity_bar_icon(self, icon_name: str):
        ...
        # ColonyForgeアイコンは下の方にあると想定
        await self.server._handle_click({"x": 25, "y": 400})
        return False

    async def test_colonyforge_sidebar_visible(self):
        """テスト: ColonyForgeサイドバーが表示されているか"""
        print("\n" + "=" * 60)
        print("テスト 1: ColonyForgeサイドバーの表示確認")
        print("=" * 60)

        # ColonyForgeアイコンをクリック
        await self.click_activity_bar_icon("ColonyForge")
        await asyncio.sleep(1)

        # キャプチャして確認
        result = await self.capture_and_describe(
            "ColonyForge Sidebar",
            "Look for ColonyForge views: Runs, Tasks, 確認要請 (requirements). Is the ColonyForge sidebar visible?",
        )

        # 結果判定（VLM応答から判断）
        analysis = result["vlm_analysis"].lower()
        passed = any(word in analysis for word in ["colonyforge", "runs", "tasks", "sidebar"])
    ...
    async def test_runs_view(self):
        ...
        result = await self.capture_and_describe(
            "Runs View",
            "Look for the Runs view in ColonyForge sidebar. Is it expanded? Are there any runs listed?",
        )
    ...
    async def test_tasks_view(self):
        ...
        result = await self.capture_and_describe(
            "Tasks View", "Look for the Tasks view in ColonyForge sidebar. Can you see task items?"
        )
    ...
    async def test_requirements_view(self):
        ...
        result = await self.capture_and_describe(
            "Requirements View", "Look for '確認要請' (requirements) view in ColonyForge sidebar."
        )
    ...
    async def test_command_palette(self):
        """テスト: コマンドパレットからColonyForgeコマンドを確認"""
        print("\n" + "=" * 60)
        print("テスト 5: コマンドパレットのColonyForgeコマンド")
        print("=" * 60)
        ...
        # "ColonyForge"と入力
        await self.server._handle_type_text({"text": "ColonyForge"})
        await asyncio.sleep(0.5)

        result = await self.capture_and_describe(
            "Command Palette - ColonyForge",
            "Look at the command palette. Are ColonyForge commands visible? List the commands you can see.",
        )
        ...
        analysis = result["vlm_analysis"].lower()
        passed = any(word in analysis for word in ["colonyforge", "dashboard", "run", "command"])
        print(f"   結果: {'✅ PASS' if passed else '❌ FAIL'}")
        result["passed"] = passed
        return result
    ...
    async def run_all_tests(self, vscode_url: str):
        """全テストを実行"""
        print("\n" + "=" * 60)
        print("🧪 ColonyForge VS Code Extension ビジュアルテスト")
        print("=" * 60)

        await self.setup(vscode_url)

        tests = [
            self.test_colonyforge_sidebar_visible,
            self.test_runs_view,
...
async def main():
    ...
    tester = ColonyForgeExtensionTest(server)
