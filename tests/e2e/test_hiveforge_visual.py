"""HiveForge VS Code拡張のE2Eビジュアルテスト

pytestで実行可能なE2Eテストスイート。
Agent UIを使用してHiveForgeのUI要素をVLMで検証します。
...
def demo_html_path() -> Generator[str, None, None]:
    ...
    # tests/e2e/から見た相対パス
    demo_html = Path(__file__).parent / "hiveforge_demo.html"

    # デモHTMLがなければ作成
    if not demo_html.exists():
        demo_html.write_text("""<!DOCTYPE html>
<html>
<head><title>HiveForge Dashboard Demo</title></head>
...
<h1>🐝 HiveForge Dashboard</h1>
<p>Welcome to HiveForge</p>
</div>
</div>
</body>
</html>""")
...
class TestHiveForgeExtensionVisual:
    """HiveForge拡張のビジュアルテストクラス"""

    @pytest.mark.asyncio
    async def test_can_navigate_to_hiveforge_page(self, agent_ui_server, demo_html_path: str):
        """HiveForgeページへ遷移できることを確認"""
        # Act
    ...
    @pytest.mark.asyncio
    async def test_vlm_recognizes_hiveforge_dashboard(self, agent_ui_server, demo_html_path: str):
        """VLMがHiveForgeダッシュボードを認識できることを確認"""
        # Arrange
        await agent_ui_server._handle_navigate({"url": demo_html_path})
        ...
        result = await agent_ui_server._handle_describe_page(
            {"focus": "What is this page? Is it HiveForge? What sections are visible?"}
        )

        # Assert
        assert len(result) > 0
        analysis = get_text_from_result(result).lower()
        # VLMがHiveForge関連のキーワードまたは画面の説明を認識すること
        expected_words = [
            "hiveforge",
            "dashboard",
            "runs",
...
class TestHiveForgeElementFinding:
    """HiveForge要素探索のテストクラス"""
    ...
    async def test_find_element_returns_result(self, agent_ui_server, demo_html_path: str):
        ...
        result = await agent_ui_server._handle_find_element(
            {"description": "HiveForge Dashboard title"}
        )
