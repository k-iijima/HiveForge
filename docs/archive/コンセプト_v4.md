# ColonyForge — Prototype Assembly（POC）v4
...
|---|---:|---|---|
| VS CodeでCopilot Chat経由でColonyForgeと対話できる | 🟡 | MCP実装: `src/colonyforge/mcp_server/server.py` / ツール定義: `src/colonyforge/mcp_server/tools.py` | Copilot側のMCP設定手順をv4（またはQUICKSTART）に明文化。必要なら「v3互換ツール名」提供も検討。 |
...
| 任意の成果物から「なぜ」を遡及できる（因果リンク） | ✅ | Lineage(MCP): `src/colonyforge/mcp_server/handlers/lineage.py` / テスト: `tests/test_mcp_server.py` | Issue #001の「自動parents付与」と現状の「任意指定parents」の整合を取る（仕様統合）。 |
| 沈黙（タイムアウト）が検出され、記録・停止が発動する | ✅ | 沈黙検出: `src/colonyforge/silence.py` / テスト: `tests/test_silence.py` | “停止が発動”の範囲（Run中断/イベントのみ/通知）をv4で厳密化。 |
| 緊急停止（EmergencyStop）が機能する | ✅ | MCP緊急停止テスト: `tests/test_mcp_server.py` / APIルート: `src/colonyforge/api/routes/runs.py` / 拡張のRun中止: `vscode-extension/src/commands/runCommands.ts` | scope=systemの扱い（POCでどこまで止めるか）をv4で明確化。 |
...
- ハッシュ計算は `src/colonyforge/core/events.py` の `compute_hash()` で行う
...
- `src/colonyforge/core/ar/storage.py` の `AkashicRecord.append()` が、追記時に `prev_hash` を自動設定する
...
- MCPツール `get_lineage(event_id, direction, max_depth)` を提供する
  ...
  - `max_depth`: 探索深度の上限
- 実装は `src/colonyforge/mcp_server/handlers/lineage.py`
  - 祖先探索: `parents` を辿るBFS
...
- `parents` が未指定（空）の場合は、**ハンドラー側で自動補完**される
  - API: `src/colonyforge/api/routes/tasks.py`, `src/colonyforge/api/routes/runs.py`
  - MCP: `src/colonyforge/mcp_server/handlers/task.py`, `src/colonyforge/mcp_server/handlers/run.py`
...
- Activity Barに `ColonyForge` コンテナを持つ
- 主要View（TreeView）:
  - Runs: `colonyforge.runs`
  - Tasks: `colonyforge.tasks`
  - 確認要請: `colonyforge.requirements`
  - Events: `colonyforge.events`
...
- `colonyforge.showDashboard` は現状 “開発中” 表示のプレースホルダー
