# ColonyForge — Prototype Assembly（POC）v3
...
| **Core** | ColonyForgeのバックエンド。AR書き込み権限を持つ唯一のコンポーネント |
...
> **POCの成果物は「VS Code内で完結するColonyForgeシステム」である。**
...
- ColonyForgeが「別のアプリを自律生成してデプロイする」のはPhase 1以降
...
- [x] VS CodeでCopilot Chat経由でColonyForgeと対話できる
...
- 本番SLA・法務レベル監査
- コスト最適化（暴走防止は設ける）
- ColonyForgeが別アプリを生成・デプロイする（Phase 1）
...
| **ツール呼び出し** | CopilotがColonyForgeのツールを自動で選択・実行 |
...
> **ColonyForge CoreはMCP Serverとしてツールを公開し、GitHub Copilotから直接呼び出される。**
  ...
  "mcp": {
    "servers": {
      "colonyforge": {
        "type": "stdio",
        "command": "python",
        "args": ["-m", "colonyforge.mcp_server"],
        "env": {
          "COLONYFORGE_VAULT_PATH": "./Vault"
        }
      }
    }
  }
...
```
User: @colonyforge 新しい要求を登録して。ユーザー認証機能を追加したい
...
```
[Copilot Chat] ──MCP──▶ [ColonyForge MCP Server] ──▶ [Core] ──追記──▶ [AR events/]
                                                      │
...
auth:
  enabled: true
  api_key_env: "COLONYFORGE_API_KEY"  # 環境変数から読み込み
```
...
| `colonyforge.config.yaml` | `*_env` で環境変数名を参照 | コミット可 |
...
|------|-------------------------|
| 要求投入 | `@colonyforge 新機能を登録: ログイン機能` |
| 状態確認 | `@colonyforge 現在のタスク状況は？` |
| 承認 | `@colonyforge decision-001 を承認して` |
| 因果追跡 | `@colonyforge task-042 はなぜ作られた？` |
| 緊急停止 | `@colonyforge 緊急停止！` |
| イベント検索 | `@colonyforge 直近の失敗イベントを見せて` |
...
#### 12.2.2 監視UI: VS Code拡張（ColonyForge Extension）
...
│  └─────────────────┘   │                                     │
├─────────────────────────┴─────────────────────────────────────┤
│  [ステータスバー]  🐝 ColonyForge: 🟢 稼働中 | Tasks: 3/12    │
└───────────────────────────────────────────────────────────────┘
```
...
1. **MCP Server**（ColonyForgeツールをCopilot Chatに公開）
...
**拡張機能ID（予定）:** `colonyforge.colonyforge-vscode`

### B.2 GitHub Copilot SDK について

> **GitHub Copilot SDKはTechnical Preview段階。ColonyForgeはMCP Serverとして実装し、Copilot Chat経由で呼び出される形を採用。**

| 方式 | 説明 | POCでの採用 |
|------|------|-------------|
| MCP Server | ColonyForgeをMCP Serverとして実装、Copilot Chatから呼び出し | ✓ 採用 |
...
# 5. Vaultディレクトリを初期化
python -m colonyforge init

# 6. MCP Server + APIサーバーを起動
python -m colonyforge serve
  ...
  "mcp": {
    "servers": {
      "colonyforge": {
        ...
        "cwd": "${workspaceFolder}",
        "env": {
          "COLONYFORGE_VAULT_PATH": "${workspaceFolder}/Vault"
        }
      }
    }
  }
...
```
User: @colonyforge システムの状態を教えて

Copilot: ColonyForgeシステムの状態を確認します。
...
User: @colonyforge 新しい要求を登録: Hello Worldアプリを作成
