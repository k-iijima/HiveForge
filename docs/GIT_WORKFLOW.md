...
| ブランチ | 寿命 | 保護 | マージ先 |
|---------|------|------|---------|
| `master` | 永続 | ✅ 保護（force push 禁止、直接コミット禁止） | — |
| `develop` | 永続 | ✅ 保護（必須レビュー + ステータスチェック） | `master`（リリース時） |
| `feat/…` | **短命**（1 Colony = 1〜3日が目安） | ❌ | `develop`（PR経由） |
| `fix/…` | 短命 | ❌ | `develop` |
| `hotfix/…` | 短命 | ❌ | `master` + `develop`（cherry-pick） |
...
| `develop` / `master` は Worktree で切り出さない | 保護ブランチはメインツリーで操作 |
...
| `develop` → `master` | `merge --no-ff` | リリース境界をマージコミットで明示 |
| `hotfix/…` → `master` | `merge --no-ff` | 修正点を明確にする |
...
| `develop` → `master` | Queen Bee | guard-l1/l2 + forager + sentinel 全通過 |
| `hotfix/…` → `master` | Queen Bee | guard-l1/l2 + sentinel 通過 |
...
gh pr merge --auto --merge     # 共有ブランチ / master へのマージの場合
```
...
|------------------|---------|-----------|---------|
| Create a merge commit | ✅ 保存 | 原著者 | `develop` → `master` |
...
| # | 禁止事項 | 理由 |
|---|---------|------|
| 1 | `master` への直接コミット | 保護ブランチ。PR経由のみ |
