...
| **Beekeeper** | ユーザー窓口・Hive統括 | ✅ M2-2完了 | — |
| **Queen Bee** | Colony統括・タスク分解 | ✅ M4-1完了 | TaskPlannerによるLLMタスク分解＋依存分析実装済 |
| **Worker Bee** | タスク実行（MCPサブプロセス） | ✅ 完了 | — |
| **Sentinel Hornet** | Hive内監視・異常検出・強制停止 | ✅ M3-6完了 | 7検出パターン + KPI劣化検出 + ロールバック/隔離 |
| **Honeycomb** | 実行履歴・学習基盤 | ✅ M3-1完了 | — |
...
| **Forager Bee** | 探索的テスト・影響分析・違和感検知 | ✅ M3-4完了 | AgentRunner統合済み |
...
│   ├── explorer.py       # 探索実行（AgentRunner統合）
...
| M2: 接続 | Sentinel Hornet、VS Code↔API、エージェント間E2E、LiteLLM、GitHub Projection | ✅ 完了（M2-0〜M2-4全完了） |
...
| G-10 | E2Eエージェントチェーンが動作 | Beekeeper→Queen→Worker完走 | ✅ (M2-2) |
