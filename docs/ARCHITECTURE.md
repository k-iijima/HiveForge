...
- **Fail-Fast（フォールバック禁止）**: 暗黙のフォールバックは導入しない。エラーは早期に例外として伝搬させ、問題の発見を遅らせない（詳細は [AGENTS.md 原則3](../AGENTS.md) を参照）
...
```
【データ階層】                          【エージェント階層】
Hive（プロジェクト）                     Beekeeper（Hive統括）
 ├── Colony（専門領域チーム）             ├── Sentinel Hornet（監視・執行）
 │    ├── Run（実行単位）                │
 │    │    ├── Task（個別タスク）         ├── Colony
 │    │    └── Requirement（確認要請）    │    ├── Queen Bee（Colony統括）
 │    └── Run ...                       │    └── Worker Bee × N（実務）
 └── Colony                             └── Colony ...
```
...
| エージェント | 操作対象 | 関係 |
|---|---|---|
| **Queen Bee** | Colony / Run | RunをWorker Beeに割り当て、進捗を統括 |
| **Worker Bee** | Run / Task | 割り当てられたRunのTaskを順次実行し、成果物を報告 |
| **Guard Bee** | Task成果物 | Worker Beeの成果物をEvidence-first検証 |
| **Forager Bee** | Task影響範囲 | 変更の影響を探索的にテスト |
...
### 3.5 Worker Bee — Run/Task の関係

Worker BeeはColonyに所属し、Queen Beeから**Run単位**で作業を割り当てられます：

```
Queen Bee ──assign──▶ Worker Bee ──execute──▶ Run
                                                 ├── Task 1 ✅
                                                 ├── Task 2 🔄
                                                 └── Task 3 ⏳
```

- **割り当て**: `worker.assigned` イベントで Worker Bee に Run が割り当てられる（`payload.run_id`, `payload.worker_id`）
- **Task実行**: Worker Bee は Run 内の Task を順次実行し、`task.assigned` → `task.completed` / `task.failed` のライフサイクルを駆動
- **報告**: 各 Task の成果物（diff, テスト結果）を `worker.completed` イベントで Queen Bee に報告
- **N案並列**: 同一 Run に対し複数の Worker Bee を割り当て、Referee Bee による選抜を行うことも可能
