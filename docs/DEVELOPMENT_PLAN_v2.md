...
| タスク | 内容 |
|--------|------|
| M3-4-a | KPI劣化検出パターンの追加（Honeycomb連携） |
| M3-4-b | ロールバックアクションの実装 |
| M3-4-c | 隔離アクションの実装 |
| M3-4-d | 新検出パターン・執行アクションのテスト追加 |
...
#### M3-5: Waggle Dance（ワグルダンス / 構造化I/O検証）

**優先度: 低** — M3-1〜M3-3完了後

| タスク | 内容 |
|--------|------|
| M3-5-a | 通信メッセージのPydanticスキーマ定義 |
| M3-5-b | 検証ミドルウェアの実装 |
| M3-5-c | 検証エラーのARイベント記録 |

**完了条件**:
- エージェント間の全通信がWaggle Danceを通過する
- スキーマ違反が自動検出・記録される

#### M3-6: Scout Bee（偵察蜂 / 編成最適化）

**優先度: 低** — Honeycombに十分なデータが蓄積されてから

| タスク | 内容 |
|--------|------|
| M3-6-a | 類似エピソード検索ロジック |
| M3-6-b | テンプレート成功率分析 |
| M3-6-c | Beekeeperへの最適化提案統合 |
...
```
     高
      ...
      │  M3-4 (Sentinel Hornet拡張) ← v1.5執行機能追加
      │  M4-1 (タスク分解)           ← 本来の価値提供
      │  M3-5 (Waggle Dance)       ← I/O構造化
      │  M3-6 (Scout Bee)           ← 最適化（データ蓄積後）
      │  M1-3 (型安全)              ← 長期信頼性（並行）
      │  M5   (運用品質)            ← プロダクション
     低
```
