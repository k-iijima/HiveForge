...
| 検出パターン | 説明 | 検出方法 |
|-------------|------|----------|
| **無限ループ** | 同一タスクの繰り返し実行、同じエラーの反復 | タスク完了/失敗パターンの周期性検出 |
| **暴走** | 異常に大量のタスク生成、制御不能な再帰 | 単位時間あたりのイベント発行レート閾値 |
| **コスト超過** | LLM APIコールの累積コストが予算を超過 | トークン消費量・API呼び出し回数の追跡 |
| **セキュリティ侵害** | ActionClass=Irreversibleの無許可実行、機密データアクセス | Action Class × Trust Level違反の検出 |
| **沈黙** | エージェントが長時間応答しない | ハートビート監視（既存Silence Detectorと連携） |
| **リソース枯渇** | ファイルシステム大量書き込み、メモリ/CPU異常消費 | システムメトリクス監視 |

**強制停止フロー**:

```
Sentinel Hornet: 異常検出
    │
    ├─► sentinel.alert_raised イベント発行（ARに記録）
    │     - alert_type: "infinite_loop" | "runaway" | "cost_overrun" | ...
    │     - target: colony_id / agent_id
    │     - evidence: 検出根拠データ
    │
...
- 停止閾値はBeekeeper経由でユーザーが調整可能（`hiveforge.config.yaml`）
...
**Action Classの自動判定ルール**:
```python
def classify_action(tool_name: str, params: dict) -> ActionClass:
    if tool_name in ["read_file", "grep_search", "semantic_search"]:
        return ActionClass.READ_ONLY
    elif tool_name in ["create_file", "edit_file"] and is_git_tracked():
        return ActionClass.REVERSIBLE
    elif tool_name in ["run_sql", "deploy", "publish"]:
...
> **⚠️ 注**: 実装計画は [DEVELOPMENT_PLAN_v1.md](DEVELOPMENT_PLAN_v1.md) の**マイルストーンベース計画（M1〜M4）** に移行済みです。
> 以下のPhase 0〜7は**ビジョンとしてのフェーズ構想**を記録として残すものであり、現在の開発タスク管理には使用しません。
> 現在の正確な進捗・タスク定義は DEVELOPMENT_PLAN_v1.md を参照してください。
...
タスク:
- [x] Hive/Run/Taskの関係をER図で整理
- [x] エージェント間メッセージ型を定義（TypeScript/Python両方）
- [x] 最小ユースケースのシーケンス図作成
- [x] 既存ARスキーマへの影響調査
- [x] **Decision Protocol設計**: 提案→決定→適用→超代の意思決定ライフサイクル
- [x] **Project Contract設計**: 構造化されたコンテキスト共有スキーマ
- [x] **Action Class定義**: Read-only/Reversible/Irreversible操作分類とTrust Level連携
- [x] **Conflict Detection設計**: Colony間意見衝突の検出・調停ルール

**成功基準 (DoD)**:
- [x] 設計ドキュメントがレビュー済み
- [x] 破壊的変更がないことを確認
- [x] Phase 1のタスクがIssue化済み
...
タスク:
- [x] `ColonyStartedEvent`, `ColonyCompletedEvent` イベント型追加
- [x] Colony状態機械の実装（IDLE → ACTIVE → COMPLETED）
- [ ] Queen BeeのMCPツール実装（`create_colony`, `query_colony`）
- [ ] VS Code拡張にColony TreeView追加
- [ ] **Conference ライフサイクルイベント追加** (`conference.started`, `conference.ended`)
- [ ] **Decision Protocol イベント追加** (`proposal.created`, `decision.recorded`, `decision.applied`)
- [ ] **Action Class モデル追加** (Read-only/Reversible/Irreversible)
...
- [ ] **Conflict Detection 機能** (`conflict.detected` イベント、調停フロー)
- [ ] **統合ポリシー定義**: 矛盾する意見のマージルール
...
5. [x] 設計ドキュメントのレビュー・承認 ← **完了 (2026-02-01)**
6. [ ] Phase 1のタスクをIssue化 ← **次のステップ**
