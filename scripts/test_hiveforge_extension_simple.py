#!/usr/bin/env python3
"""HiveForge拡張テストのシンプル実行スクリプト

使い方:
    # code-serverを使う場合
    python scripts/test_hiveforge_extension_simple.py --url http://localhost:8080
...
<html>
<head>
    <title>HiveForge Dashboard Demo</title>
    <style>
        body { 
...
<body>
    <div class="container">
        <div class="activity-bar">
            ...
            <div class="activity-icon active" title="HiveForge">🐝</div>
            <div class="activity-icon" title="Extensions">🧩</div>
        </div>
        <div class="sidebar">
            <div class="sidebar-header">HiveForge</div>
        ...
        <div class="main-content">
            <h1 class="dashboard-title">🐝 HiveForge Dashboard</h1>
            <p>Welcome to HiveForge - AI-Powered Development Orchestration</p>
...
async def run_test(url: str, headless: bool = False):
    ...
    print("\n" + "=" * 60)
    print("🐝 HiveForge Extension Visual Test")
    print("=" * 60)
    print(f"URL: {url}")
    ...
    # ステップ 1: ページを開く
    print("\n📍 Step 1: Open HiveForge Dashboard")
    ...
    describe_result = await server._handle_describe_page(
        {
            "focus": "Describe the HiveForge dashboard. What views are visible? (Runs, Tasks, Requirements)"
        }
    )
    ...
    # ステップ 4: 要素を探す
    print("\n🔍 Step 4: Find HiveForge Icon")
    find_result = await server._handle_find_element(
        {"description": "HiveForge icon (bee icon 🐝) in the activity bar"}
    )
...
async def main():
    parser = argparse.ArgumentParser(description="HiveForge Extension Visual Test")
