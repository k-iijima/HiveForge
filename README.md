...
**M1〜M4 完了** — 基盤固め、接続、適応的協調、自律的タスク分解が全て実装済。現在はM5残項目（運用品質）とM6（要求分析Colony）を推進中。
...
| M6: 要求分析 | 🔄 M6-1 一部完了（モデル+永続化）、M6-2〜M6-5 設計済 |
...
│   ├── events/        #   81 EventType (パッケージ化済)
│   ├── models/        #   ドメインモデル (ActionClass等)
│   ├── ar/            #   Akashic Record (JSONL永続化)
│   ├── state/         #   6状態機械 (Run/Task/Requirement/Hive/Colony/RA)
...
├── sentinel_hornet/   # Sentinel Hornet（5検出+KPI劣化+ロールバック/隔離）
