"""ColonyForge 設定管理モジュール

Pydantic Settingsを使用した型安全な設定管理。
colonyforge.config.yaml と環境変数から設定を読み込む。
"""
...
class AuthConfig(BaseModel):
    ...
    api_key_env: str = Field(default="COLONYFORGE_API_KEY")
...
class GitHubConfig(BaseModel):
    ...
    label_prefix: str = Field(
        default="colonyforge:",
        description="ColonyForge管理ラベルのプレフィックス",
    )


class ColonyForgeSettings(BaseSettings):
    """ColonyForge全体設定

    設定の優先順位:
    1. 環境変数
    2. colonyforge.config.yaml
    3. デフォルト値
    """

    model_config = SettingsConfigDict(
        env_prefix="COLONYFORGE_",
    ...
    @classmethod
    def from_yaml(cls, config_path: Path | str | None = None) -> "ColonyForgeSettings":
        """YAMLファイルから設定を読み込む
        ...
        Returns:
            ColonyForgeSettings インスタンス
        """
        if config_path is None:
            # デフォルトの設定ファイルパスを探索
            search_paths = [
                Path.cwd() / "colonyforge.config.yaml",
                Path.cwd() / "colonyforge.config.yml",
                Path.home() / ".colonyforge" / "config.yaml",
            ]
...
# グローバル設定インスタンス（遅延初期化）
_settings: ColonyForgeSettings | None = None


def get_settings() -> ColonyForgeSettings:
    """設定シングルトンを取得"""
    global _settings
    if _settings is None:
        _settings = ColonyForgeSettings.from_yaml()
    return _settings


def reload_settings(config_path: Path | str | None = None) -> ColonyForgeSettings:
    """設定を再読み込み"""
    global _settings
    _settings = ColonyForgeSettings.from_yaml(config_path)
    return _settings
