...
if TYPE_CHECKING:
    from hiveforge.prompts.loader import (
        BeekeeperConfig,
        QueenBeeConfig,
        WorkerBeeConfig,
    )
...
WORKER_BEE_SYSTEM = """\
You are a Worker Bee in HiveForge — a specialist agent that executes concrete tasks.
...
QUEEN_BEE_SYSTEM = """\
You are a Queen Bee in HiveForge — the colony coordinator \
...
BEEKEEPER_SYSTEM = """\
You are a Beekeeper in HiveForge — the user-facing interface \
...
def get_prompt_from_config(
    ...
    Returns:
        System prompt string
    """
    from hiveforge.prompts.loader import PromptLoader
